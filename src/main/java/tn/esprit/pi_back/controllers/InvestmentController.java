...
public class InvestmentController {
    ...
    @GetMapping("/offers")
    public ResponseEntity<List<InvestmentOffer>> getMyOffers() {
        Long userId = userService.getCurrentUserOrThrow().getId();
        return ResponseEntity.ok(investmentService.getOffersForUser(userId));
    }

    @PostMapping("/offers/{offerId}/accept")
    public ResponseEntity<InvestmentOffer> acceptOffer(@PathVariable Long offerId) {
        return ResponseEntity.ok(investmentService.acceptOffer(offerId));
    }

    @GetMapping("/offers/{offerId}/performance")
    public ResponseEntity<List<PerformanceTracking>> getPerformance(@PathVariable Long offerId) {
