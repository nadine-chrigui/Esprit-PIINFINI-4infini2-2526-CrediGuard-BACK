...
import tn.esprit.pi_back.repositories.LoyaltyLevelRepository;
...
public class FinanceService {
    ...
    public CompteFinancierResponse getCompte(Long id) {
        ...
        ensureCardExists(compte);
        return toCompteResponse(compte);
    }

    @Transactional
    public CompteFinancierResponse getCompteByUtilisateurId(Long utilisateurId) {
        CompteFinancier compte = compteFinancierRepository.findByUtilisateurId(utilisateurId)
    ...
    private CompteFinancier ensureCardExists(CompteFinancier compte) {
        if (compte.getCarte() == null) {
            Carte carte = new Carte();
            carte.setCardNumber(generateCardNumber());
            
            User owner = compte.getUtilisateur();
            String holderName = (owner != null && owner.getFullName() != null) 
                    ? owner.getFullName().toUpperCase() 
                    : (compte.getFirstName() != null ? (compte.getFirstName() + " " + compte.getLastName()).toUpperCase() : "CLIENT CREDIGUARD");
            
            carte.setCardHolderName(holderName);
            carte.setCvv(String.format("%03d", new java.util.Random().nextInt(1000)));
            carte.setExpiryDate(java.time.LocalDate.now().plusYears(4));
            carte.setActive(true);
            carte.setCompte(compte);
            
            ensureBronzeLevel();
            loyaltyLevelRepository.findByName("BRONZE").ifPresent(carte::setLoyaltyLevel);
            
            carteRepository.save(carte);
            compte.setCarte(carte);
        }
        
        // Defensive: ensure credit fields are not null for existing accounts
        if (compte.getCreditLimite() == null) compte.setCreditLimite(1000.0);
        if (compte.getCreditDisponible() == null) compte.setCreditDisponible(compte.getCreditLimite());
        if (compte.getCreditUtilise() == null) compte.setCreditUtilise(0.0);
        if (compte.getSolde() == null) compte.setSolde(0.0);
    ...
    public CompteFinancierResponse createCompte(CompteFinancierRequest request) {
        ...
        if (request.getUtilisateurId() != null) {
            utilisateur = userRepository.findById(request.getUtilisateurId()).orElse(null);
        }

        if (utilisateur == null && request.getEmail() != null) {
            Optional<User> existingUser = userRepository.findByEmail(request.getEmail());
            if (existingUser.isPresent()) {
                utilisateur = existingUser.get();
            } else {
                utilisateur = new User();
                utilisateur.setFullName(request.getFirstName() + " " + request.getLastName());
                utilisateur.setEmail(request.getEmail());
                utilisateur.setPhone(request.getPhone());
                utilisateur.setPassword("password123");
                utilisateur.setUserType(UserType.CLIENT);
        ...
        if (utilisateur == null) {
            throw new RuntimeException("Could not find or create user for financial account");
        }

        Optional<CompteFinancier> existingCompte = compteFinancierRepository.findByUtilisateurId(utilisateur.getId());
        if (existingCompte.isPresent()) {
            return toCompteResponse(existingCompte.get());
        }
        ...
        // Initialize credit fields to avoid null issues
        compte.setCreditLimite(1000.0); // Default limit
        compte.setCreditDisponible(1000.0);
        compte.setCreditUtilise(0.0);
        
        compte = compteFinancierRepository.save(compte);

        // --- AUTOMATIC CARD GENERATION ---
        Carte carte = new Carte();
        carte.setCardNumber(generateCardNumber());
        carte.setCardHolderName(utilisateur.getFullName() != null ? utilisateur.getFullName().toUpperCase() : "CLIENT CREDIGUARD");
        carte.setCvv(String.format("%03d", new java.util.Random().nextInt(1000)));
        carte.setExpiryDate(java.time.LocalDate.now().plusYears(4));
        carte.setActive(true);
        carte.setCompte(compte);
        
        // Default Loyalty Level (e.g., BRONZE)
        ensureBronzeLevel();
        loyaltyLevelRepository.findByName("BRONZE").ifPresent(carte::setLoyaltyLevel);
    ...
    private void ensureBronzeLevel() {
        if (loyaltyLevelRepository.findByName("BRONZE").isEmpty()) {
            LoyaltyLevel bronze = new LoyaltyLevel();
            bronze.setName("BRONZE");
            bronze.setCashbackRate(1.0);
            bronze.setSpendingThreshold(0.0);
            bronze.setCardColor("#080f18");
            bronze.setBenefits("Standard Benefits");
            loyaltyLevelRepository.save(bronze);
        }
    }
    ...
    private CompteFinancierResponse toCompteResponse(CompteFinancier compte) {
        CompteFinancierResponse res = CompteFinancierResponse.builder()
        ...
        if (compte.getCarte() != null) {
            res.setCardNumber(compte.getCarte().getCardNumber());
            res.setCardHolderName(compte.getCarte().getCardHolderName());
            res.setCvv(compte.getCarte().getCvv());
            res.setExpiryDate(compte.getCarte().getExpiryDate() != null ? compte.getCarte().getExpiryDate().toString() : null);
            if (compte.getCarte().getLoyaltyLevel() != null) {
                res.setCardColor(compte.getCarte().getLoyaltyLevel().getCardColor());
            }
        }
        return res;
    }
