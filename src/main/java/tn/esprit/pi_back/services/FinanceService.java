...
public class FinanceService {
    ...
    public CompteFinancierResponse getCompteByUtilisateurId(Long utilisateurId) {
        ...
        if (!currentUser.getId().equals(utilisateurId) && currentUser.getUserType() != UserType.ADMIN) {
            throw new SecurityException("Unauthorized access to another user's financial account");
        }
    ...
    public CompteFinancierResponse createCompte(CompteFinancierRequest request) {
        User currentUser = userService.getCurrentUserOrThrow();
        User utilisateur = currentUser;

        if (currentUser.getUserType() == UserType.ADMIN && request.getUtilisateurId() != null) {
